...
{
    public class ChargeRiposte : BaseSkillState
    {
        ...
        public override void OnEnter()
        {
            ...
            Util.PlaySound(ChargeRiposte.enterSFXString, base.gameObject);
            this.soundID = Util.PlaySound(ChargeRiposte.startChargeLoopSFXString, base.gameObject);
        }
        ...
        public override void OnExit()
        {
            if ((bool)this.chargeVfxInstanceTransform)
            {
                ...
                this.PlayAnimation("Gesture, Additive", ChargeRiposte.EmptyStateHash);
                this.PlayAnimation("Gesture, Override", ChargeRiposte.EmptyStateHash);
                this.crosshairOverrideRequest?.Dispose();
                this.chargeVfxInstanceTransform = null;
            }
            base.characterMotor.walkSpeedPenaltyCoefficient = 1f;
            Util.PlaySound(ChargeRiposte.endChargeLoopSFXString, base.gameObject);
            base.OnExit();
        }
        ...
        public override void FixedUpdate()
        {
            ...
            if (this.charge >= ChargeRiposte.minChargeForChargedAttack && !this.chargeVfxInstanceTransform && (bool)ChargeRiposte.chargeVfxPrefab)
            {
                if ((bool)ChargeRiposte.crosshairOverridePrefab && this.crosshairOverrideRequest == null)
                {
                    this.crosshairOverrideRequest = CrosshairUtils.RequestOverrideForBody(base.characterBody, ChargeRiposte.crosshairOverridePrefab, CrosshairUtils.OverridePriority.Skill);
                }
                Transform transform = base.FindModelChild(ChargeRiposte.chargeVfxChildLocatorName);
                if ((bool)transform)
                {
                    this.chargeVfxInstanceTransform = UnityEngine.Object.Instantiate(ChargeRiposte.chargeVfxPrefab, transform).transform;
                    ScaleParticleSystemDuration component = this.chargeVfxInstanceTransform.GetComponent<ScaleParticleSystemDuration>();
                    if ((bool)component)
                    {
                        component.newDuration = (1f - ChargeRiposte.minChargeForChargedAttack) * this.chargeDuration;
                    }
                }
                base.PlayCrossfade("Gesture, Additive", ChargeRiposte.ChargePunchIntroStateHash, ChargeRiposte.ChargePunchIntroParamHash, this.chargeDuration, 0.1f);
                base.PlayCrossfade("Gesture, Override", ChargeRiposte.ChargePunchIntroStateHash, ChargeRiposte.ChargePunchIntroParamHash, this.chargeDuration, 0.1f);
            }
            if ((bool)this.chargeVfxInstanceTransform)
            {
