...
{
    public class ChargeSkewer : BaseSkillState
    {
        ...
        public override void OnEnter()
        {
            ...
            Util.PlaySound(ChargeSkewer.enterSFXString, base.gameObject);
            this.soundID = Util.PlaySound(ChargeSkewer.startChargeLoopSFXString, base.gameObject);
        }
        ...
        public override void OnExit()
        {
            if ((bool)this.chargeVfxInstanceTransform)
            {
                ...
                this.PlayAnimation("Gesture, Additive", ChargeSkewer.EmptyStateHash);
                this.PlayAnimation("Gesture, Override", ChargeSkewer.EmptyStateHash);
                this.crosshairOverrideRequest?.Dispose();
                this.chargeVfxInstanceTransform = null;
            }
            base.characterMotor.walkSpeedPenaltyCoefficient = 1f;
            Util.PlaySound(ChargeSkewer.endChargeLoopSFXString, base.gameObject);
            base.OnExit();
        }
        ...
        public override void FixedUpdate()
        {
            ...
            if (this.charge >= ChargeSkewer.minChargeForChargedAttack && !this.chargeVfxInstanceTransform && (bool)ChargeSkewer.chargeVfxPrefab)
            {
                if ((bool)ChargeSkewer.crosshairOverridePrefab && this.crosshairOverrideRequest == null)
                {
                    this.crosshairOverrideRequest = CrosshairUtils.RequestOverrideForBody(base.characterBody, ChargeSkewer.crosshairOverridePrefab, CrosshairUtils.OverridePriority.Skill);
                }
                Transform transform = base.FindModelChild(ChargeSkewer.chargeVfxChildLocatorName);
                if ((bool)transform)
                {
                    this.chargeVfxInstanceTransform = UnityEngine.Object.Instantiate(ChargeSkewer.chargeVfxPrefab, transform).transform;
                    ScaleParticleSystemDuration component = this.chargeVfxInstanceTransform.GetComponent<ScaleParticleSystemDuration>();
                    if ((bool)component)
                    {
                        component.newDuration = (1f - ChargeSkewer.minChargeForChargedAttack) * this.chargeDuration;
                    }
                }
                base.PlayCrossfade("Gesture, Additive", ChargeSkewer.ChargePunchIntroStateHash, ChargeSkewer.ChargePunchIntroParamHash, this.chargeDuration, 0.1f);
                base.PlayCrossfade("Gesture, Override", ChargeSkewer.ChargePunchIntroStateHash, ChargeSkewer.ChargePunchIntroParamHash, this.chargeDuration, 0.1f);
            }
            if ((bool)this.chargeVfxInstanceTransform)
            {
                base.characterMotor.walkSpeedPenaltyCoefficient = ChargeSkewer.walkSpeedCoefficient;
            }
            if (base.isAuthority)
            {
