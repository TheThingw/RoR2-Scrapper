...
namespace Scrapper.Content
{
    public static class Tokens
    {
        ...
        private static void Add(string token, string text) => LanguageManager.Add(ScrapperSurvivor.SCRAPPER_PREFIX + token.Replace(" ", "_").ToUpper(), text);
        public static string DamageText(string text) => $"<style=cIsDamage>{text}</style>";
        public static string DamageValueText(float value) => $"<style=cIsDamage>{value * 100}% damage</style>";
        public static string UtilityText(string text) => $"<style=cIsUtility>{text}</style>";
        public static string HealthText(string text) => $"<style=cIsHealth>{text}</style>";
        public static string KeywordText(string keyword, string sub) => $"<style=cKeywordName>{keyword}</style><style=cSub>{sub}</style>";
        ...
        public static string GetAchievementDescriptionToken(string identifier) => $"ACHIEVEMENT_{identifier.ToUpperInvariant()}_DESCRIPTION";
        ...
        public static void Init()
        {
            ...
            Add("KEYWORD_PREPARE", KeywordText("Prepare", "Prepare yourself."));
            Add("KEYWORD_IMPALE", KeywordText("Impale", "Impale your enemies."));
            ...
            Add("PASSIVE_DESCRIPTION", impalePrefix + "Gain stacks from Primary and Secondary up to 5 Stacks. Each stack Increases \"Impale\" damage by 25%, armor by 5, and attack speed by 7.5%. Lose stacks while out of combat.(1  Stack every 3?-10? seconds out of combat. will experiment with this one)");
            #endregion

            #region Primary
            Add("PRIMARY_THRUST_NAME", "Thrust");
            Add("PRIMARY_THRUST_DESCRIPTION", agilePrefix + $"Stab forward for 240% damage (1.0 Proc), Cycles between two stab animations. On kill Impale forward for 500% damage (1.0 Proc). Evey 5 Hits Give 1 Passive Stack.");
            #endregion

            #region Secondary
            Add("SECONDARY_QUICKSTEP_NAME", "Quick Step");
            Add("SECONDARY_QUICKSTEP_DESCRIPTION", preparePrefix + $"Sidestep, Gaining invincibility, and adding 1 Passive stack. then Lunge forward towards your crosshair for 390% (1.0 Proc). On kill Impale forward for 500% damage (1.0 Proc).");

            Add("SECONDARY_THUNDERSTEP_NAME", "Thunder Step");
            Add("SECONDARY_THUNDERSTEP_DESCRIPTION", preparePrefix + $"Blast upwards, Gaining invincibility, adding 1 Passive stack, and causing a small 300% (1.0 Proc) blast. Then lunge forward towards your crosshair for 390% (1.0 Proc)");
            #endregion
            ...
            Add("SPECIAL_EYES_DESCRIPTION", impalePrefix + $"Ready your weapon, revealing critical spots on nearby enemies, and slowing your fall speed, then make up to 2 targeted strikes for 2x390%(maybe a bit higher). Critical hits reset secondary skill cooldown. additionally Critical Kills Reduce Special Skill cooldown by 2 seconds");
            #endregion

            #region Achievements
            LanguageManager.Add(GetAchievementNameToken(ScrapperMasteryAchievement.identifier), "Scrapper: Mastery");
            LanguageManager.Add(GetAchievementDescriptionToken(ScrapperMasteryAchievement.identifier), "As Scrapper, beat the game or obliterate on Monsoon.");
            #endregion
        }
    }
}
